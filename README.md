...
# Logic:
The class Student has its default constructor defined inside the class. When an object is created, it prompts the user for details and stores them. Demonstrates how in-class constructors handle initialization.
# Algorithm:
1. Start  
2. Define a class Student with private members: name, branch, roolnos, fees.  
3. Inside the class, define a default constructor to take input for all members.  
4. In `main()`, create an object of Student.  
5. End


# **Program 2: Default Constructor (Out-class)**
# Logic:
The default constructor is declared inside the class but defined outsid using the scope resolution operator `::`. When the object is created, it accepts Student details. A separate `display()` function shows stored information, illustrating separation of declaration and definition.
...
# Algorithm:
1. Start  
2. Define a class Product with members: Pcode, name, type, price.  
...
# Logic:
This program explains how a copy constructor works. Class Studen initializes its data members with a parameterized constructor. A copy constructor duplicates data of an existing object, demonstrating object copying during creation.
# Algorithm:
1. Start
2. Define a class **Student** with members: `name`, `age`.
3. Define a parameterized constructor to assign passed arguments to data members.
4. Define a **copy constructor** to copy values from another object and display a message.
5. Define a `display()` function to show student details.
6. In `main()`, create an object `s1` using arguments and call `display()`.
7. Create another object `s2` as a copy of `s1` (invokes copy constructor) and call `display()`.
8. End
